...
fun main(args: Array<String>) {
    val url = "https://raw.githubusercontent.com/OAI/OpenAPI-Specification/master/examples/v2.0/yaml/petstore.yaml"
    generate("com.example", "PetStore", url)
}
...
private fun generateClientImpl(serviceName: String, swagger: Swagger, builder: KotlinFile.Builder) {
    ...
    swagger.paths.orEmpty().forEach { (pathStr, path) ->
        path.operationMap.orEmpty().forEach { (method, op) ->
            ...
            val bodyParam = op.parameters.find { it.`in` == "body" }
...
fun generateServer(serviceName: String, swagger: Swagger) {
    val classDecl = "class ${serviceName}Server() : HttpApi({\n"
    val routes = swagger.paths.orEmpty().flatMap { (pathStr, path) ->
        ...
        val route = "    route(HttpMethod.$method, \"$pathFixed\").handler { ctx ->\n"
        ...
        val paramDecls = params.fold("") { acc, param ->
            acc + "        val ${param.name.toCamelCase(false)} = req.getParam(\"${param.name}\")\n"
        }
        ...
        val suffix = "    }\n"
        acc + route + req + paramDecls + headerDecls + response + suffix
    }
    val suffix = "})\n"
